...
namespace GameFramework.DataTools.Editor
{
    /// <summary>
    /// XLSX 转 CSV 的通用事务包装。
    /// 转换和校验都在临时目录完成，全部通过后才更新正式 CSV。
    /// </summary>
    internal static class XlsxCsvExportPipeline
    {
        public static XlsxToCsvExportReport ExportDirectory(
            string inputDirectory,
            string outputDirectory,
            string temporaryFolderName,
            Action<IReadOnlyList<string>> validateFiles,
            SearchOption searchOption = SearchOption.TopDirectoryOnly)
        {
            ...
            if (string.IsNullOrWhiteSpace(temporaryFolderName))
            {
                throw new ArgumentException("临时目录名称不能为空。", nameof(temporaryFolderName));
            }

            if (validateFiles == null)
            {
                throw new ArgumentNullException(nameof(validateFiles));
            }

            string fullOutputDirectory = Path.GetFullPath(outputDirectory);
            string temporaryDirectory = CreateTemporaryDirectory(temporaryFolderName);

            try
            {
                XlsxToCsvExportReport temporaryReport = XlsxToCsvConverter.ConvertDirectory(inputDirectory, temporaryDirectory, searchOption);
                validateFiles.Invoke(temporaryReport.OutputFiles);
                return PublishFiles(temporaryReport, fullOutputDirectory, temporaryDirectory);
            }
            finally
            {
                TryDeleteTemporaryDirectory(temporaryFolderName, temporaryDirectory);
            }
        }

        private static XlsxToCsvExportReport PublishFiles(XlsxToCsvExportReport temporaryReport, string outputDirectory, string temporaryDirectory)
        {
            ...
            foreach (string temporaryFile in temporaryReport.OutputFiles)
            {
                ...
                entries.Add(new(temporaryFile, destinationPath, backupPath, hadOriginalFile));
            }
            ...
            try
            {
                ...
                {
                    if (FilesHaveEqualContent(entry.TemporaryPath, entry.DestinationPath))
                    {
                        continue;
                    }

                    entry.WasUpdated = true;
                    File.Copy(entry.TemporaryPath, entry.DestinationPath, overwrite: true);
                }
            }
        ...
        private static Exception TryRollback(IReadOnlyList<PublishEntry> entries)
        {
        ...
        private static string CreateTemporaryDirectory(string temporaryFolderName)
        {
            string temporaryRoot = GetTemporaryRoot(temporaryFolderName);
        ...
        private static void TryDeleteTemporaryDirectory(string temporaryFolderName, string temporaryDirectory)
        {
            try
            {
                string temporaryRoot = GetTemporaryRoot(temporaryFolderName);
        ...
        private static string GetTemporaryRoot(string temporaryFolderName)
        {
            return Path.GetFullPath(Path.Combine(Path.GetTempPath(), temporaryFolderName));
        }

        private static void EnsureChildPath(string parentDirectory, string childPath)
        {
            ...
            if (!fullChildPath.StartsWith(fullParentDirectory, StringComparison.OrdinalIgnoreCase))
            {
        ...
        {
            public PublishEntry(string temporaryPath, string destinationPath, string backupPath, bool hadOriginalFile)
            {
