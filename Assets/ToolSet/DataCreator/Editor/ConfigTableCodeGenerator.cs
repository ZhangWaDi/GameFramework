...
namespace GameFramework.ConfigData.Editor
{
    ...
    internal static class ConfigTableCodeGenerator
    {
            ...
            out int generatedScriptCount)
        {
            ...
            string normalizedScriptAssetFolder = NormalizeAssetFolderPath(
                scriptAssetFolder,
                nameof(scriptAssetFolder));
            string outputDirectory = GetAbsoluteProjectPath(
                normalizedScriptAssetFolder);
            Directory.CreateDirectory(outputDirectory);

            string normalizedPreviousFolder =
                NormalizeOptionalAssetFolderPath(previousScriptAssetFolder);
            bool outputFolderChanged =
                !string.IsNullOrEmpty(normalizedPreviousFolder) &&
                !string.Equals(
                    normalizedPreviousFolder,
                    normalizedScriptAssetFolder,
                    StringComparison.OrdinalIgnoreCase);
            string previousDirectory = outputFolderChanged
                ? GetAbsoluteProjectPath(normalizedPreviousFolder)
                : null;
            ...
            foreach (ConfigTableDefinition definition in definitions)
            {
                ...
                string dataFilePath = Path.Combine(
                    outputDirectory,
                    $"{tableName}.cs");
                string tableFilePath = Path.Combine(
                    outputDirectory,
                    $"{tableName}SO.cs");
            ...
            for (int index = 0; index < schema.Fields.Count; index++)
            {
                ...
                else
                {
                    AppendXmlSummary(
                        ...
                        indentation: "        ");
                    string typeName =
                        ConfigValueConverter.GetCSharpTypeName(field.Kind);
        ...
        private static void EnsureOwnedGeneratedFile(string filePath)
        {
                    ...
                    StringComparison.Ordinal))
            {
                throw new InvalidOperationException(
                    $"文件“{filePath}”不包含自动生成标记，已停止覆盖或删除。" +
                    "请修改输出目录，或手动确认该同名文件的所有权。");
            }
        }
            ...
            string parameterName)
        {
            ...
            {
                throw new ArgumentException(
                    $"SO 脚本输出目录必须位于 Assets 下：{assetFolderPath}",
                    parameterName);
            }
            ...
            foreach (string segment in normalized.Split('/'))
            {
                ...
                {
                    throw new ArgumentException(
                        $"SO 脚本输出目录不能包含相对路径段：{assetFolderPath}",
                        parameterName);
                }
            }
            ...
            string indentation)
        {
            ...
            foreach (string line in NormalizeLineBreaks(effectiveDescription)
                         .Split('\n'))
            {
                source.AppendLine(
